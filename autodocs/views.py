    ...
    def resolve_patterns(patterns,ancestor=[]):
            for p in patterns:
                if type(p) == urlresolvers.RegexURLPattern:
                    if hasattr(p, '_callback_str'):
                        p.callbackstr = p._callback_str
                    elif p.name == 'index':
                        p.callbackstr = 'apps.main.main_views.default_page'
                    else:
                        p.callbackstr = ''
                    req = ancestor+[p]
                    res.append(req)
...
def import_module_without_decorators(mod,func):
    ...
    pyfile = sub_suffix(ori_mod.__file__)
    f = open(pyfile,'r')
    fread = f.read()
    f.close()
    newmodfile = pyfile.replace('.py','_temp.py')
    f2 = open(newmodfile,'w')
    f2.write(fread.replace(r'@',r'#@'))
    f2.close()
    new_module = import_module(mod+'_temp')
    res = getattr(new_module, func)
    os.remove( newmodfile )
    return res
